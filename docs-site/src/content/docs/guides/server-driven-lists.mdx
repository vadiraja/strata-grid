...
For **total count and loading state**, the cleanest pattern today is to
let your adapter expose them. The adapter is where `loadPage` runs — it
already knows the count and the in-flight state. Expose a tiny subscribe
seam from the adapter that your status bar reads:
...
Your status bar reads from `subscribe()` and re-renders. This keeps the
grid component pure and the data-source observable, which is the right
separation. A built-in `onPaginationChange` callback on `<DataGrid>` is
tracked as a future ergonomic addition.
...
Planned ergonomic additions (no breaking changes):

- **`onPaginationChange`** — surface `{ currentPage, totalCount, isLoading }`
  from the grid directly, removing the need for the adapter-subscribe
  pattern in Step 5.
...
- **Discrete `onSortChange` / `onFilterChange` / `onSearchChange`** —
  surgical callbacks for finer URL serialization than `onViewStateChange`.
